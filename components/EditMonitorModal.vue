...
async function handleSubmit() {
  ...
  try {
    ...
    errors.value.general = err.message || 'Failed to update monitor'
  } finally {
...
<template>
  <Dialog :open="open" title="Edit Monitor" @update:open="handleOpenChange">
    <form class="space-y-4" @submit.prevent="handleSubmit">
      ...
      <div class="space-y-1.5">
        <Label for="edit-url">{{ form.type === 'http' ? 'URL' : 'Host:Port' }}</Label>
        <Input
          id="edit-url"
          v-model="form.url"
          :placeholder="form.type === 'http' ? 'https://example.com' : 'example.com:80'"
          class="font-mono"
